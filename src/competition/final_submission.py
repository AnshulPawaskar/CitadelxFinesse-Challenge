...
OOS_START_DATE = datetime(2026, 1, 1)
...
def run():
    ...
    result = build_static_portfolio(selected, history, actual_formation_date, full_end_date, initial_capital=10_00_00_000)
    validate_no_rebalancing(result["trade_log"])

    backtest_metrics = compute_metrics(result["daily_nav"].filter(pl.col("Datetime") <= COMPETITION_END_DATE), result["initial_capital"])
    oos_nav = result["daily_nav"].filter(pl.col("Datetime") >= OOS_START_DATE)
    oos_metrics = (
        compute_metrics(oos_nav, oos_nav["PortfolioValue"][0]) if oos_nav.height > 1
        else {"Note": "No OOS 2026 H1 data available for this formation date."}
    )

    benchmark = build_synthetic_benchmark(history)
    bench_window = benchmark.filter(
        (pl.col("Datetime") >= result["portfolio_start"]) & (pl.col("Datetime") <= COMPETITION_END_DATE)
    ).sort("Datetime")
    ...
    final_portfolio = build_final_portfolio_report(selected, features, history, actual_formation_date, universe)

    print(f"Formation date: {actual_formation_date}")
    print(final_portfolio)
    print("\n--- Competition Backtest Performance (through 2025-12-31) ---")
    ...
    print("\n--- Out-of-Sample Performance (2026 H1) ---")
    ...
    result["trade_log"].write_csv(RESULTS_DIR / "competition_trade_log.csv")
    result["daily_nav"].write_csv(RESULTS_DIR / "competition_daily_nav.csv")
    contributions.write_csv(RESULTS_DIR / "competition_stock_contributions.csv")
